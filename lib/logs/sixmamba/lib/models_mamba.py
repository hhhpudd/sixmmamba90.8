...
def vim_small_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_rope_also_residual(pretrained=False, **kwargs):
    ...
    if pretrained:
#        checkpoint = torch.hub.load_state_dict_from_url(
#            url="/home/WuXD/HiyaS_Python_Code/MambaFuse/pretrained/vim_s_midclstok_80p5acc.pth",
#            map_location="cpu", check_hash=True
#        )
        checkpoint = torch.load('pretrained/vim_s_midclstok_80p5acc.pth')
        model.load_state_dict(checkpoint["model"])
    return model
...
def vim_small_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, embed_dim=384, depth=12, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='none', if_abs_pos_embed=False, if_rope=False, if_rope_residual=False, bimamba_type="v2", if_cls_token=True, if_devide_out=True, use_middle_cls_token=True,  **kwargs)
        #depth 24 -> 12
    model.default_cfg = _cfg()
    if pretrained:
#        checkpoint = torch.hub.load_state_dict_from_url(
...
#        )
        #checkpoint = torch.load('pretrained/vim_s_midclstok_80p5acc.pth')
        checkpoint = torch.load('pretrained/vim_s_midclstok_ft_81p6acc.pth')
        model.load_state_dict(checkpoint["model"],strict=False)
#    pe = model.pos_embed[:, 1:, :].detach()
#    pe = pe.transpose(-1, -2)
#    pe = pe.view(pe.shape[0], pe.shape[1], int(np.sqrt(pe.shape[2])), int(np.sqrt(pe.shape[2])))
#    pe = F.interpolate(pe, size=(14,14), mode='bilinear', align_corners=True)  # Notice: 352/16 = 22, 384/16 = 24
#    pe = pe.flatten(2)
#    pe = pe.transpose(-1, -2)
#    model.pos_embed = nn.Parameter(pe)
    model.head = nn.Linear(384, 196 * 768)
