...
class SelfAttention(nn.Module):
    def __init__(self, input_dim, num_heads=2, max_len=1, device=None):
    ...
    def forward(self, x):
        ...
        return out
#     # def __init__(self, input_dim, max_len=1, device=None):
#     #     super(SelfAttention, self).__init__()
#     #
...
#     #
#     # def forward(self, x):
#     #     # Reshape the input tensor to (batch_size, num_channels, num_pixels)
#     #     batch_size, num_channels, width, height = x.size()
#     #     x = x.reshape(batch_size, num_channels, -1)
#     #
...
#     #     scores = scores / (num_channels ** 0.5)
#     #     scores = self.softmax(scores)
#     #
#     #     # Weighted sum of values using attention scores
#     #     out = torch.bmm(scores, value.permute(0, 2, 1))
#     #
#     #     # Reshape the output tensor to original shape
...
class FAFusion_block(nn.Module):
    def __init__(self, ch_1, ch_2,  ch_int, ch_out, drop_rate=0., scale_size=1, ring_width = 2, imgsize = 24, ring_radius = 12):
        ...
        # self.hight_block = AxialAttention(ch_int, ch_int, groups=4, kernel_size=24 * self.scale_size)
        # self.width_block = AxialAttention(ch_int, ch_int, groups=4, kernel_size=24 * self.scale_size,stride=1, width=True)
        # self.diagonal_block1 = AxialAttention(ch_int, ch_int, groups=4,kernel_size=24 * self.scale_size,stride=1)
        # self.diagonal_block2 = AxialAttention(ch_int, ch_int, groups=4, kernel_size=24 * self.scale_size+1,stride=1
        if (not self.ring_radius is 0) and(not self.ring_width is 0):
            self.ring_attention = RingAttention(in_planes=ch_int, out_planes=ch_int,groups=4, ring_width=ring_width, stride=1, imgsize=imgsize, radius=self.ring_radius)
        self.conv_up = conv1x1(ch_int, ch_int)
        self.bn2 = nn.BatchNorm2d(ch_int)

        # bi-linear modelling for both
#        self.W_g = Conv(ch_1, ch_int, 1, bn=True, relu=False)  # Conv1x1
#        self.W_t = Conv(ch_2, ch_int, 1, bn=True, relu=False)  # Conv1x1
#        self.W = Conv(ch_int, ch_int, 3, bn=True, relu=True)   # Conv3x3

	# CHANGES
	#self.residual = Residual(ch_int, ch_out) # This is True
        #self.residual = Residual(ch_int*2, ch_out)# AbStudy
    ...
    def forward(self, g, t):
        # Hadamard product Pi of Ti and Gi
#        W_g = self.W_g(g)
#        W_t = self.W_t(t)
#        bp_m = self.W(W_g * W_t)
...
#        print(bp.shape)
        bp = self.convpro(bp)
        bp = self.conv_down1(bp)     # Integrated channel information
        bp = self.bn1(bp)
        bp = self.relu(bp)
#        print("after MSGC")
#        print(bp.shape)
...
class FAFuse_B(nn.Module):
    def __init__(self, num_classes=1, drop_rate=0.2, normal_init=True, pretrained=False, ring_radius=[12,24,48], ring_width=[4,8,16]):
        ...
        # 调整了等比
        self.up_c = FAFusion_block(ch_1=256, ch_2=768, ch_int=256, ch_out=256, drop_rate=drop_rate / 2, scale_size=1, ring_width=ring_width[0], imgsize=14, ring_radius=ring_radius[0]) # 384x384:[24, 48, 96]  352x352:[22,44,88] 224x224:[14,28,56]
        self.up_c_1_1 = FAFusion_block(ch_1=128, ch_2=128, ch_int=128, ch_out=128, drop_rate=drop_rate / 2, scale_size=2, ring_width=ring_width[1], imgsize=28, ring_radius=ring_radius[1])
        self.up_c_1_2 = Up(in_ch1=256, out_ch=128, in_ch2=128, attn=True)
        self.up_c_2_1 = FAFusion_block(ch_1=64, ch_2=64, ch_int=64, ch_out=64, drop_rate=drop_rate / 2, scale_size=4, ring_width=ring_width[2], imgsize=56, ring_radius=ring_radius[2])
    ...
    def forward(self, imgs, labels=None):
        ...
        return map_x, map_1, map_2    # middle，left，right
